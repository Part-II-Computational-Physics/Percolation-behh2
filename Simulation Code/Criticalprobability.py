...
# Code used to estimate the critical probabilty for site percolation in 2D
...
p_av = [] #average occupation probability at which percolation occurs
error = []
...
for i in Lattice_sizes:
    ...
    for k in range(10):
        ...
        std = 0
        for j in range(1000):
            # run programme for k=1000 permuations for each lattice size and find average n at which percolation occurs
            lattice.nearestneighbour(sample)
            lattice.permutate(sample)
            Percolation = lattice.percolate(sample)[1]
            n.append((np.where(Percolation == 1)[0][0]))

        n_av = np.mean(n)
        p.append(n_av / i ** 2)
    p_av.append(np.mean(p))
    error.append(np.std(p))

data = np.array([Lattice_sizes, p_av, error])
np.save('paverage.npy', data)
